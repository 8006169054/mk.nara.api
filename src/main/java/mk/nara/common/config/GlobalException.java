...
//      return KainosResponseEntity.builder().failResponse(new KainosException(ex, HttpStatus.INTERNAL_SERVER_ERROR)).build().colse();
	...
	private ResponseEntity<?> dataIntegrityViolationException(DataIntegrityViolationException ex) {
		 return KainosResponseEntity.builder()
				 	.status("E")
	        		.message(KainosMessageString.builder()
	        				.message("동일한 데이터가 존재합니다.")
	        				.messageType(KainosKey.Status.Error)
	        				.build())
	        		.build().colse();
	}
